...
def train_and_predict_dragons(t, y_unscaled, x, net, targeted_regularization=True, output_dir='',
    ...
    return test_outputs, train_outputs, net
...
def run_ihdp(data_base_dir='/Users/claudiashi/data/ihdp_csv', output_dir='~/result/ihdp/',
    ...
    simulation_files = sorted(glob.glob("{}/*.csv".format(data_base_dir)))[:5]
    ...
    for idx, simulation_file in enumerate(simulation_files):
        ...
        np.savez_compressed(os.path.join(simulation_output_dir, "simulation_outputs.npz"),
                            t=x_t, y=y_t, y_cf=y_cf_t, mu_0=mu_0_t, mu_1=mu_1_t)
        ...
        for is_targeted_regularization in [False]:
            ...
            _, _,net = train_and_predict_dragons(t_s, y_s, x_s,net,
            ...
            if dragon == 'tarnet':
                print('I am here making tarnet')
                net = TarNet_transfer(x.shape[1],parm).cuda()

            elif dragon == 'dragonnet':
                print("I am here making dragonnet")
                net = DragonNet_transfer(x.shape[1],parm).cuda()
            ### step 2 ###
            test_outputs, train_output,net = train_and_predict_dragons(t_t, y_t, x_t,net,
                                                                   targeted_regularization=is_targeted_regularization,
                                                                   knob_loss=knob_loss, ratio=test_ratio, dragon=dragon,
